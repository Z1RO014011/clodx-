...
export function StatusDock({ snapshot, onOpen }: { snapshot?: ProviderSnapshot; onOpen: () => void }) {
  ...
  return <button className="status-dock" onClick={onOpen} aria-label="打开 Clodx 额度详情"><span className="dock-glyph">›_</span><span className="dock-values">{short && <strong>5H {formatPercent(short.remainingPercent)}</strong>}<strong>WK {weekly ? formatPercent(weekly.remainingPercent) : '--'}</strong></span></button>;
}
