...
- 按 Codex 当前账户实际返回的窗口显示 **5 小时额度** 或 **周额度** 的剩余百分比和刷新时间
- 显示当前可用的 **额度重置次数**
- 悬浮状态窗显示紧凑信息：`5H` 优先；没有 5H 窗口时自动显示 `WK`
- 原生托盘菜单：显示/隐藏、立即刷新、退出
- 启动后静默驻留菜单栏，点击图标即可打开或隐藏详情
- 支持 macOS 和 Windows

## 最近更新与背景

Codex 的额度页面会因套餐、账户和产品调整而呈现不同的窗口组合。部分账户当前只显示 **1 周额度** 和 **可用重置次数**，不再提供独立的 5 小时窗口。

Clodx 现在按照接口中的窗口时长分类，而不是把主窗口固定写成 5H：

- 18,000 秒窗口显示为 `5H`
- 604,800 秒窗口显示为 `WK`
- 只有周额度时，主界面、悬浮状态窗和菜单栏都会显示周额度；例如 `WK 97%`
- 可用重置次数会与额度窗口一起刷新，但应用不会自动使用任何重置次数

开发启动也已简化：执行一次 `npm run tauri dev` 即会自动启动 Vite 与桌面程序。正式版保持静默驻留菜单栏，不会在启动时抢占屏幕。
...
该命令会自动启动 Vite 开发服务器和 Clodx；无需另开终端。
